...
public class BobOmbEntity extends Monster {
	...
	public static void addLivingEntityToBiomes(BiomeLoadingEvent event) {
		if (SPAWN_BIOMES.contains(event.getName()))
			event.getSpawns().getSpawner(MobCategory.MONSTER).add(new MobSpawnSettings.SpawnerData(SuperMarioModEntities.BOB_OMB.get(), 20, 4, 4));
	}

	public BobOmbEntity(PlayMessages.SpawnEntity packet, Level world) {
		this(SuperMarioModEntities.BOB_OMB.get(), world);
	}

	public BobOmbEntity(EntityType<BobOmbEntity> type, Level world) {
		super(type, world);
		xpReward = 0;
		setNoAi(false);
	}
