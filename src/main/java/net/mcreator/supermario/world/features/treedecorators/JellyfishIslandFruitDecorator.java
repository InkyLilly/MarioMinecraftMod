...
public class JellyfishIslandFruitDecorator extends CocoaDecorator {
	public static final JellyfishIslandFruitDecorator INSTANCE = new JellyfishIslandFruitDecorator();
	public static com.mojang.serialization.Codec<JellyfishIslandFruitDecorator> codec;
	public static TreeDecoratorType<?> tdt;
	static {
		codec = com.mojang.serialization.Codec.unit(() -> INSTANCE);
		tdt = new TreeDecoratorType<>(codec);
		tdt.setRegistryName("jellyfish_island_tree_fruit_decorator");
		ForgeRegistries.TREE_DECORATOR_TYPES.register(tdt);
	}

	public JellyfishIslandFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (random.nextFloat() <= 0.25F) {
						...
						if (Feature.isAir(level, blockpos)) {
							biConsumer.accept(blockpos, SuperMarioModBlocks.JELLYFISH_ISLAND_FRUIT.get().defaultBlockState());
						}
					}
				}
			});
