...
public class MechaKoopaRangedItemItem extends Item {
	public MechaKoopaRangedItemItem() {
		super(new Item.Properties().tab(SuperMarioModTabs.TAB_MARIO_ENTITIES).stacksTo(1));
	}
			...
			if (true) {
				...
				if (stack == ItemStack.EMPTY) {
					for (int i = 0; i < entity.getInventory().items.size(); i++) {
						...
						if (teststack != null && teststack.getItem() == SuperMarioModItems.MECHA_KOOPA_RANGED_ITEM.get()) {
							stack = teststack;
							break;
						}
					}
				...
				if (entity.getAbilities().instabuild || stack != ItemStack.EMPTY) {
					...
					if (entity.getAbilities().instabuild) {
						...
						if (new ItemStack(SuperMarioModItems.MECHA_KOOPA_RANGED_ITEM.get()).isDamageableItem()) {
